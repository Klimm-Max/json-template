...
import io.github.matoklimm.functions.Count
...
class CountTest {
    ...
    fun `first argument is used as a step argument`() {
        for (i in 2..100 step 2) {
            val result = fn.execute(mutableListOf("2"), ctx).toInt()
            assertEquals(i, result)
        }
    }
    ...
    fun `first arg is used as start value and second arg is used as step`() {
        var result = fn.execute(mutableListOf("10", "2"), ctx).toInt()
        assertEquals(10, result)    // first invocation, we use start index from arg
        for (i in 12..100 step 2) {
            result = fn.execute(mutableListOf("10", "2"), ctx).toInt() // since the start index was already used, just increase by step 2
            assertEquals(i, result)
        }
    }
    ...
    fun `first arg is close to LONG_MAX_VALUE will overflow`() {
        val result = fn.execute(mutableListOf("9223372036854775806", "3"), ctx).toLong()
        assertEquals(9223372036854775806, result)
        val result2 = fn.execute(mutableListOf("9223372036854775806", "3"), ctx).toLong()
    ...
    fun `passing too big args throws`() {
        ...
        assertFailsWith<IllegalArgumentException> {
            fn.execute(mutableListOf("9223372036854775808", "2"), ctx)
        }
    }
}